...
public class Facade {
        ...
        if (verificadorKey(key)) {
            if (veriTipoUsuario(key, "1")) {
                ...
                for (String relacion : relacionador) {
                    ...
                    if(relacionador.contains(nit))
                    {
                        response="Esta empresa ya existe"; 
                        break; 
                    }
                    if (data[1].equals(key)) {
                        username = data[0];
                ...
                dataRelacionador.saveRelaEmpresaUsuario(username, nit+"/"); 
            
                Empresa emp = dataEmpresas.getE er);
    ...
    private String createOfertaBase(String key, EmpresaFactory empresa) {
        String response = "";
        if (verificadorKey(key)) {
            if (veriTipoUsuario(key, "1")) {
                String username = "";
                ...
                Oferta oferta= new (id); 
                relacionador.add(uData);
                dataRelacionador.saveRelaOfertaUsuario(username, oferta); 
